...
async function remove(id: string) { const affected = await db.ideas.where('tagIds').equals(id).count(); if (!confirm(`删除后将从 ${affected} 条灵感中移除该标签，确定继续吗？`)) return; await repository.remove(id); await load() }
onMounted(load)
</script>
<template><main class="page tags-page"><header><h1>标签</h1><p>给灵感建立轻量分类。</p></header><TagManager :tags="tags" @create="create" @rename="rename" @remove="remove" /><p v-if="message" role="alert">{{ message }}</p></main></template>
