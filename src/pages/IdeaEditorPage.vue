...
<template><main class="page"><header class="editor-header"><button type="button" aria-label="返回" @click="router.back()">←</button><h1>新增灵感</h1></header><div class="mode-tabs"><button type="button" :aria-pressed="mode === 'text'" @click="mode = 'text'">文字记录</button><button type="button" :aria-pressed="mode === 'voice'" @click="mode = 'voice'">语音记录</button></div><template v-if="mode === 'voice'"><VoiceRecorder @started="startTranscription" @stopped="stopTranscription" @recorded="audio = $event" @cancel="audio = undefined; transcription.cancel()" /><TranscriptEditor :status="transcriptStatus" :text="transcript" :interim="interim" @update="updateTranscript" /></template><IdeaForm :key="initial.body" :initial="initial" :busy="busy" :allow-empty-body="Boolean(audio)" @change="saveDraft" @save="save" /></main></template>
