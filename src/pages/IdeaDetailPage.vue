...
onMounted(async () => { const id = String(route.params.id); const [ideaItem, assets, tagItems] = await Promise.all([repository.get(id), contentRepository.getAssets(id), createTagRepository(db).list()]); idea.value = ideaItem; audio.value = assets.audio; transcript.value = assets.transcript; tags.value = tagItems; if (!idea.value) { error.value = '灵感不存在或已被删除'; return }; if (audio.value) audioUrl.value = URL.createObjectURL(audio.value.blob) })
...
async function updateTranscript(text: string, manual: boolean) { if (!transcript.value) return; transcript.value = { ...transcript.value, text, manuallyEdited: manual, updatedAt: new Date().toISOString() }; await contentRepository.updateTranscript(transcript.value) }
</script>
