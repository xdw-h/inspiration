...
onMounted(async () => { const id = String(route.params.id); idea.value = await repository.get(id); if (!idea.value) { error.value = '灵感不存在或已被删除'; return }; audio.value = await db.audioAssets.where('ideaId').equals(id).first(); transcript.value = await db.transcripts.where('ideaId').equals(id).first(); if (audio.value) audioUrl.value = URL.createObjectURL(audio.value.blob) })
